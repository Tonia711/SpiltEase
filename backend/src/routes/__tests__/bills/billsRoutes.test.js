...
afterAll(async () => {
  if (testBill1) {
    try {
      await request(app).delete(
        `/api/bills/${testGroup._id}/bill/${testBill1._id}`
      );
    } catch (error) {
  ...
  if (testBill2) {
    try {
      await request(app).delete(
        `/api/bills/${testGroup._id}/bill/${testBill2._id}`
      );
    } catch (error) {
...
describe("Bill API Routes", () => {
  ...
  it("should update a bill", async () => {
    ...
    const bill = updatedBill.groupBills.find(
      (b) => b._id.toString() === testBill1._id.toString()
    );
  ...
  it("should delete a bill", async () => {
    ...
    const updatedBill = await Bill.findOne({ groupId: testGroup._id });
    const stillExists = updatedBill.groupBills.find(
      (b) => b._id.toString() === testBill2._id.toString()
    );
