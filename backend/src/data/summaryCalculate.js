...
export const calculateExpenseSummary = async (group, userId, billsData) => {
  try {
    ...
    if (
      !billsData ||
      !billsData.groupBills ||
      billsData.groupBills.length === 0
    ) {
      return {
        groupSummary: [],
        userSummary: [],
    ...
    console.log(
      `User numeric memberId for expense matching: ${userNumericMemberId}`
    );
    ...
    for (const bill of billsData.groupBills) {
      ...
      if (!labelInfo) {
        console.log(
          `No label info found for numeric ID ${labelNumericId} (from ${labelIdStr}) on bill ${bill.id}`
        );
        continue;
      }
      ...
      if (!userEntry) {
        const userMemberIdStr = userMember._id
          ? userMember._id.toString()
          : null;
      ...
      if (userEntry) {
        ...
        console.log(
          `User with memberId ${userNumericMemberId} not found in bill ${bill.id} members list`
        );
      }
    }

    console.log(
      "Group summary before formatting:",
      Object.entries(groupSummaryMap).map(
        ([labelKey, data]) =>
          `${labelKey} (${data.labelName}): ${data.totalExpense} - ${data.totalRefund} = ${data.netExpense}`
      )
    );

    console.log(
      "User summary before formatting:",
      Object.entries(userSummaryMap).map(
        ([labelKey, data]) =>
          `${labelKey} (${data.labelName}): ${data.userExpense} - ${data.userRefund} = ${data.netExpense}`
      )
    );
    ...
    const groupSummary = Object.entries(groupSummaryMap)
      .map(([labelKey, item]) => ({
        ...
        labelName:
          item.labelName.charAt(0).toUpperCase() + item.labelName.slice(1),
        totalExpense: parseFloat(item.netExpense.toFixed(2)),
        percentage:
          groupTotal > 0
            ? parseFloat(((item.netExpense / groupTotal) * 100).toFixed(2))
            : 0,
      }))
    ...
    const processedUserSummary = Object.entries(userSummaryMap)
      .map(([labelKey, item]) => ({
        ...
        percentage:
          userTotal > 0
            ? parseFloat(((item.netExpense / userTotal) * 100).toFixed(2))
            : 0,
      }))
