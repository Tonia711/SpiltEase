      ...
      <div className={styles.container}>
        <div className={styles.header}>
          ...
          <div onClick={handleGroupClick}>
            ...
            <div className="group-id" style={{ fontSize: "0.7rem", color: "#888" }}>
              ID: {group._id}
            </div>
          </div>
        </div>
          ...
          <ul>
            {bills.map((bill) => (
              <li key={bill._id}>
                ...
                <div>{bill.label?.type || "Unnamed Expense"}</div>
                <div>💰 Amount: ${bill.expenses ?? "N/A"}</div>
                <div>📅 Date: {bill.date ? new Date(bill.date).toLocaleDateString() : "N/A"}</div>
              </li>
            ))}
          </ul>
        )}
